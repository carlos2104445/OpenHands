    ...
    async def ainit(self):
        ...
        if 'agent_skills' in self.plugins and 'jupyter' in self.plugins:
            obs = await self.run_ipython(
                IPythonRunCellAction(
                    code='from openhands.runtime.plugins.agent_skills.agentskills import __all__ as _agent_skills_all\n'
                    'from openhands.runtime.plugins.agent_skills import agentskills\n'
                    'for _func_name in _agent_skills_all:\n'
                    '    globals()[_func_name] = getattr(agentskills, _func_name)\n'
                    'del _agent_skills_all, _func_name\n'
                )
            )
        ...
        if not browser_observation.error:
            ...
            if not new_downloads:
                ...
                if len(downloaded_file_paths) == 1:
                    content = f'Execution of the previous action {action.browser_actions} resulted in a file download. The downloaded file is saved at location: {downloaded_file_paths[0]}'
                else:
                    file_list = ', '.join(downloaded_file_paths)
                    content = f'Execution of the previous action {action.browser_actions} resulted in {len(downloaded_file_paths)} file downloads. The downloaded files are saved at locations: {file_list}'
